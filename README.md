...
### Polling Rate
Note different zone modules handle polling differently. For example, QuickZone has a time-based polling system, while ZoneService has a frame-based one. In order to make the tests fair each zone library was configured such that they have similar polling frequencies. In the tests above ZoneService was configured with a polling interval of 2 frames, meaning it scans the same subject every other frame. QuickZone, on the other hand, was configured with a 30hz polling rate. As a result, ZoneService scanned entities more frequently in every test, as in order to drop below 30hz with a 2-frame interval ZoneService would have to run at less than 60 FPS.
